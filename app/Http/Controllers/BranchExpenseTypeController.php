...
use App\Models\BranchExpenseType;
use App\Models\ExpenseType;
use Illuminate\Http\Request;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\DB;

class BranchExpenseTypeController extends CommonPivot
{
    use Helpers;
    ...
    public function show(Request $request,string $val): Response
    {
        if(str_starts_with($val,'_')){
            ...
            if($request->user()->manager==="No") {
                $selected = DB::select("select brt.id, rt.name from branch_expense_types brt inner join expense_types rt on rt.id = brt.exp_id where brt.branch_id=".$request->user()->branch_id);
            }
            else{
