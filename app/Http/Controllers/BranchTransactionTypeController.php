...
use App\Models\BranchTransactionType;
use App\Models\TransactionType;
use Illuminate\Http\Request;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\DB;

class BranchTransactionTypeController extends CommonPivot
{
    use Helpers;
    ...
    public function show(Request $request,string $val): Response
    {
        if(str_starts_with($val,'_')){
            ...
            if($request->user()->manager==="No") {
                $selected = DB::select("select brt.id, rt.name from branch_transaction_types brt inner join transaction_types rt on rt.id = brt.tr_id where brt.branch_id=".$request->user()->branch_id);
            }
            else{
